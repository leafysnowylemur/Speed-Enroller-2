...
`simpleMode = ""` 
This is an extention to `testing` and make it so that nothing is set for the `username` and `password` and sets the `targetTime` to 1.5 minutes ahead. This mode wont make it past the OKTA login screen but is useful for testing wheither your browser paths and settings are configured correctly.


`id = "" ` 
This is your username to login to ERNIE
`pw = "" ` 

This is your password to log into ERNIE
